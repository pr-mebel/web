...
export const initFileUpload = createAction(
    'init_file_upload',
    (file: { name: string }) => ({
        payload: file.name,
    })
        ...
        filesArray.forEach((file, index) => {
            ...
            uploadTask.on(
                ...
                (snapshot) => {
                    ...
                    dispatch(
                        updateFileUploadProgress(file.name, index, progress)
                    );
                },
                (error) =>
                    dispatch(setErrorInFileUpload(file.name, index, error))
            );
        });
    }
);
...
export default createReducer(initialState, (builder) => {
    builder
        .addCase(
            saveForm,
            (state, { payload: { name, tel, email, description } }) => {
                state.name = name || '';
                state.email = email || '';
                state.tel = tel || '';
        ...
        .addCase(
            ...
            (state, { payload: { index, progress, name } }) => {
                state.files = [
        ...
        .addCase(
            setErrorInFileUpload,
            (state, { payload: { index, name, error } }) => {
                state.files = [
                    ...state.files.slice(0, index),
                    { name, error },
                    ...state.files.slice(index + 1),
